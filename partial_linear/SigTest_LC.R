...
TSLC <- foreach(k = 1:B_rep,
                  .multicombine=TRUE, .packages=c('isoSurv', 'survival', 'pracma', 'MASS'),
                  .options.snow=opts) %dopar% {
                    ...
                    Linear_covariates <- mvrnorm(sampsize_ori, mu = c(0,0), Sigma = matrix(c(1, 0, 0, 1), nrow = 2, byrow = FALSE))
                    W <- as.data.frame(Linear_covariates)
                    ...
                    test_cox_Tcox <- Power_unicox_vs_multicox(Z, phiZ = hzrate1, W = W, beta_trt = c(1, 2), B = B_size, samp_size = sampsize_ori, k = k, qt = 0.93)#0.93
                    #Monotone hazard model vs partial linear model
                    test_pl_Tcox <- Power_mono_vs_partial(Z, phiZ = hzrate1, W = W, beta_trt = c(1, 2), B = B_size, samp_size = sampsize_ori, k = k, qt = 0.93)#0.93
                    
                    #when true model for hazard is partial linear model
                    #Univariate Cox model vs multivariate Cox model
                    test_cox_Tpl <- Power_unicox_vs_multicox(Z, phiZ = hzrate_m1, W = W, beta_trt = c(1, 2), B = B_size, samp_size = sampsize_ori, k = k, qt = 0.9)#0.9
                    #Monotone hazard model vs partial linear model
                    test_pl_Tpl <- Power_mono_vs_partial(Z, phiZ = hzrate_m1, W = W, beta_trt = c(1, 2), B = B_size, samp_size = sampsize_ori, k = k, qt = 0.9)#0.9
...
save(TSLC, file = "PowerComp100s100iter100bt_2LinearCov.rda")
