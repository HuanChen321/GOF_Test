...
```{r}
plot(x = HFData$serum_creatinine,  y = HFData$time)
lines(x = HFData$serum_creatinine[HFData$DEATH_EVENT > 0], y = HFData$time[HFData$DEATH_EVENT > 0], type = 'p', col = 'red')
```
...
```{r}
plot(x = iso$iso.cov$covs + iso$Zk, y = psi_hat, 
     xlab = 'Serum Creatinine (Z)', 
     ylab = expression(paste("Fitted Value for ", phi(Z))), 
     col = "red", type = "s", pch = 20, lty = 1)
lines(x = iso$iso.cov$covs + iso$Zk, y = mybeta*(iso$iso.cov$covs - iso$Zk), col = "blue", type = "l", lty = 2, pch = 20)
legend("topleft", legend = c("monotone hazard model", "Cox model"), lty = c(1,2), col = c("red", "blue"), cex = 1.2)
```
...
```{r}
coxph(Surv(time, status-1)~., cancer)
```

```{r}
data(cancer, package="survival")
HCData <- na.omit(data.frame("stop" = cancer$time, "covs" = -cancer$pat.karno, "event" = cancer$status-1))

fit_lung <- coxph(Surv(stop, event)~., HCData)
summary(fit_lung)
...
```{r}
plot(x = -(MySimu$covs), y = psi_hat, 
     xlab = 'Karnofsky Performance Score (Z)', 
     ylab = expression(paste("Fitted Value for ", phi(Z))), 
     ylim = c(-0.2,1.2),
     col = "red", type = "s", pch = 20, lty = 1)
lines(x = -(MySimu$covs ), y = -mybeta*(-MySimu$covs+iso$Zk), col = "blue", type = "l", lty = 2, pch = 20)
legend("topright", legend = c("Isotonic Proportional Hazards Model", "Cox Model"), lty = c(1,2), col = c("red", "blue"), cex = 1)
```
...
sampsize_ori = dim(MGData)[1] #500
  ...
  MySimu_ori <- data.frame("stop" = MGData$futime, "covs" = MGData$mspike, "event" = MGData$death)
...
```{r}
plot(x = (MySimu$covs), y = psi_hat, 
     xlab = 'Monoclonal Serum Spike (Z)', 
     ylab = expression(paste("Fitted Value for ", phi(Z))), 
     col = "red", type = "s", pch = 20, lty = 1)
lines(x = (MySimu$covs ), y = mybeta*(MySimu$covs - iso$Zk), col = "blue", type = "l", lty = 2, pch = 20)
legend("topleft", legend = c("Isotonic Proportional Hazards Model", "Cox Model"), lty = c(1,2), col = c("red", "blue"), cex = 1.2)
```
...
plot(x = (MySimu$covs), y = psi_hat, 
     xlab = 'Age (Z)', 
     ylab = expression(paste("Estimated Value for ", phi(Z))), 
     col = "red", type = "s", pch = 20, lty = 1)
lines(x = (MySimu$covs ), y = mybeta*(MySimu$covs - iso$Zk), col = "blue", type = "l", lty = 2, pch = 20)
legend("topleft", legend = c("Isotonic Proportional Hazards Model", "Cox Model"), lty = c(1,2), col = c("red", "blue"), cex = 1.2)
```
...
```
```{r}
1-mean(mydata_tind$death) # censoring rate
```
...
sampsize_ori = dim(aids.id)[1] 
  ...
  MySimu_ori <- data.frame("stop" = aids.id$Time, "covs" = -aids.id$CD4, "event" = aids.id$event)
...
```{r}
#censoring rate
1-mean(aids.id$event)
```

```{r}
plot(x = -(MySimu$covs + median(MySimu$covs)), y = psi_hat, 
     xlab = 'CD4 Cell Count (Z)', 
     ylab = expression(paste("Fitted Value for ", phi(Z))), 
     col = "red", type = "s", pch = 20, lty = 1)
lines(x = -(MySimu$covs + isofit$Zk), y = mybeta*(MySimu$covs - iso$Zk), col = "blue", type = "l", lty = 2, pch = 20)
...
# sampsize_ori = dim(GBSG2)[1] 
sampsize_ori = dim(gbsg)[1] 
  ...
  MySimu_ori <- data.frame("stop" = gbsg$rfstime, "covs" = gbsg$nodes, "event" = gbsg$status)
...
```{r}
plot(x = MySimu$covs + median(MySimu$covs), y = psi_hat, 
     xlab = 'Number of Positive Nodes (Z)', 
     ylab = expression(paste("Fitted Value for ", phi(Z))), 
     ylim = c(0,3),
     col = "red", type = "s", pch = 20, lty = 1)
lines(x = MySimu$covs + isofit$Zk, y = mybeta*(MySimu$covs - iso$Zk), col = "blue", type = "l", lty = 2, pch = 20)
legend("bottomright", legend = c("Isotonic Proportional Hazards Model", "Cox Model"), lty = c(1,2), col = c("red", "blue"), cex = 1.2)
```
## Significance test
```{r}
summary(cox)
...
```{r}
cox_sq <- coxph(Surv(stop, event) ~ (MySimu$covs)^2, data = MySimu, control = coxph.control(timefix = tf), ties = "breslow")
summary(cox_sq)
```

```{r}
cox_ln <- coxph(Surv(stop, event) ~ log(MySimu$covs), data = MySimu, control = coxph.control(timefix = tf), ties = "breslow")
summary(cox_ln)
```

```{r}
cox_sqrt <- coxph(Surv(stop, event) ~ sqrt(MySimu$covs), data = MySimu, control = coxph.control(timefix = tf), ties = "breslow")
summary(cox_sqrt)
```

```{r}
library(pracma)
cox_3rt <- coxph(Surv(stop, event) ~ nthroot(MySimu$covs, 3), data = MySimu, control = coxph.control(timefix = tf), ties = "breslow")
summary(cox_3rt)
```



## lileihood ratio test for log linearity
https://stats.stackexchange.com/questions/370301/check-log-linearity-of-a-continuous-variable-to-predict-survival
...
cox_mod1 <- coxph(Surv(rfstime, status) ~ nodes, data = gbsg)

# fit a Cox model with a nonlinear effect for 'risk_factor' using natural cubic splines
cox_mod2 <- coxph(Surv(rfstime, status) ~ ns(nodes, 4), data = gbsg)
...
ND <- with(gbsg, data.frame(nodes = seq(min(gbsg$nodes),max(gbsg$nodes), length.out = 500)))
...
xyplot(pred + lo + up ~ nodes, data = ND,
...
# fit <- cox.aalen(Surv(rfstime, status)~prop(nodes), data = gbsg, weighted.test = 0, residuals = 1, rate.sim=0,n.sim=500)
fit <- aalen(Surv(rfstime, status)~nodes, data = gbsg, n.sim = 1000, residuals = 1)
summary(fit)
```
...
fit.mg<-cum.residuals(fit,data=gbsg,n.sim=500,
modelmatrix=model.matrix(~-1+factor(cut(nodes,3)),gbsg))
par(mfrow=c(1,4))
...
```{r}
# residual.fit = cum.residuals(fit, data = gbsg, cum.resid = 1, n.sim = 100)
# cumulative residuals vs. covariates Lin, Wei, Ying style
fit.mg<-cum.residuals(fit,data=gbsg,cum.resid=1,n.sim=1000)
par(mfrow=c(1,1))
plot(fit.mg,score=2)
summary(fit.mg)
```
...
```{r}
fit <- cox.aalen(Surv(rfstime, status)~prop(nodes), data = gbsg, residuals = 1, rate.sim=0,n.sim=500)
summary(fit)
```

```{r}
resids.ut = cum.residuals(fit, data=gbsg, cum.resid = 1, n.sim = 500)
plot(resids.ut, score = 2, xlab = "nodes")
summary(resids.ut)
```
## Including spline 
```{r}
mfit <- coxph(Surv(rfstime, status) ~ pspline(nodes,df = 5), data = gbsg)
mfit
```
