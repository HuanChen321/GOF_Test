...
Bootstrap_c_exp <- function(Z, hzrate, lambda, B = 100, samp_size = 500){
...
for (i in 1:B) {
  ...
  survtime_B <- -log(runif(samp_size))/(lambda*exp(hzrate_B))
  ...
  # if(i %% 100 == 0){
  #   Ave.time = (Sys.time() - start.time)/i
  #   print(i)
  #   print(Sys.time() + (B - i)*Ave.time)
  # }
}
